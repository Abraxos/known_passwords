...
def insert_password(cursor, password):
    sha = sha512_hash(password)
    q = SQL("INSERT INTO passwords.password (passwd, sha512) SELECT {0},"
            "{1} WHERE NOT EXISTS (SELECT * FROM passwords.password WHERE "
            "sha512 = {1} OR passwd = {0}) RETURNING passwd_id")\
        .format(Literal(password), Literal(sha))
    cursor.execute(q)
    r = cursor.fetchone()
    if r:
        return r[0]
    else:
        cursor.execute(SQL("SELECT passwd_id FROM passwords.password "
                           "WHERE sha512={0}").format(Literal(sha)))
        r = cursor.fetchone()
        if r:
            return r[0]


def connect_acct_passwd(cursor, acct_id, passwd_id):
    if acct_id and passwd_id:
        q = SQL("INSERT INTO passwords.accounts_passwords (acct_id, "
                "passwd_id) SELECT {0},{1} WHERE NOT EXISTS (SELECT * FROM "
                "passwords.accounts_passwords WHERE acct_id={0} AND "
                "passwd_id={1}) RETURNING rel_id")\
            .format(Literal(acct_id), Literal(passwd_id))
        cursor.execute(q)
        r = cursor.fetchone()
        if r:
            return r[0]
        else:
            cursor.execute(SQL("SELECT rel_id FROM "
                               "passwords.accounts_passwords "
                               "WHERE acct_id={0} AND passwd_id={1}")
                           .format(Literal(acct_id), Literal(passwd_id)))
            r = cursor.fetchone()
            if r:
                return r[0]
...
def account_passwords_to_database(acct_psswd_pairs,
    ...
    for acct, passwd in acct_psswd_pairs:
        ...
        connect_acct_passwd(cursor,
                            insert_account(cursor, acct),
                            insert_password(cursor, passwd))
    conn.commit()
    conn.close()
